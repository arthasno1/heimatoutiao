...
export default {
  ...
  async created() {
    ...
    console.log('CommentList的res,created触发', res)
  ...
  methods: {
    timeAgo,
    async loveFn(boolean, commentObj) {
      if (boolean === true) {
        // 取消点赞
    ...
    // 点击发布---发送评论
    async sendFn() {
      const res = await sendCommentAPI({ id: this.$route.query.art_id, content: this.comText })
      console.log('发布评论axios', res)
    ...
    async onLoad() {
      if (this.commentArr.length > 0) {
        ...
        if (res.data.data.last_id === null) {
          this.finished = true
          console.log('CommentList的res', 'finished为true', res)
        } else {
          this.commentArr = [...this.commentArr, ...res.data.data.results]
          this.totalCount = res.data.data.total_count
          this.lastId = res.data.data.last_id
          this.loading = false
          console.log('CommentList的res', res)
        }
      } else {
        this.loading = false
      }
